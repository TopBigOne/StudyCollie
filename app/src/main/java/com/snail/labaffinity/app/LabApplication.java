    ...
    private void initCollie() {
        Collie.getInstance().init(this, new Config(true, true, true, true, true, true), new CollieListener() {
            ...
            public void onAppColdLaunchCost(long duration, String processName) {
                Log.d(TAG, " onAppColdLaunchCost app冷启动耗时 : " + duration + ", processName : " + processName);
            }

            @Override
            public void onActivityLaunchCost(Activity activity, long duration, boolean finishNow) {
                Log.d(TAG, "onActivityLaunchCost activity启动耗时 " + activity.getComponentName().getClassName() + ", " + duration + ", finishNow :" + finishNow);
            ...
            public void onLeakActivity(String activity, int count) {
                Log.d("Collie", "内存泄露 " + activity + " 数量 " + count);
            }

            @Override
            public void onCurrentMemoryCost(TrackMemoryInfo trackMemoryInfo) {
                Log.d("Collie", "onCurrentMemoryCost# 内存概况：" + trackMemoryInfo.procName + " java内存： " + trackMemoryInfo.appMemory.dalvikPss + " native内存:  " + trackMemoryInfo.appMemory.nativePss);
            }
            ...
            public void onFpsTrack(Activity activity, long currentCostMils, long currentDropFrame, boolean isInFrameDraw, long averageFps) {
                if (currentDropFrame >= 2)
                    Log.d("Collie", "Activity " + activity + " 掉帧 " + currentDropFrame + " 是否因为Choro 绘制掉帧 " + isInFrameDraw + " 1s 平均帧率" + averageFps);
            }

            @Override
            public void onANRAppear(Activity activity) {
                Log.w("Collie", "Activity " + activity + " ANR  ");
